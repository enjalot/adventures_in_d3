...
(function($) {
    $.scrollorama = function(options) {
		...
		function onScrollorama() {
			...
			var currBlockIndex = getCurrBlockIndex(scrollLeft);
			...
			for (var i=0; i<blocks.length; i++) {
				...
				if (blocks[i].animations.length) {
					for (var j=0; j<blocks[i].animations.length; j++) {
						...
						if (i > currBlockIndex) {
							if (currBlockIndex != i-1 && anim.baseline != 'right') {
						...
						// unless on an element that gets animated when it hits the bottom of the viewport
							...
							if (blocks[i].pin) {
								blocks[i].block
									.css('position', 'absolute')
									.css('left', (blocks[i].left + blocks[i].pin));
							}
						}
						...
						if (i == currBlockIndex || (currBlockIndex == i-1 && anim.baseline == 'bottom')) {
							...
							var startAnimPos = blocks[i].left + anim.delay;
							if (anim.baseline == 'bottom')  startAnimPos -= $(window).width();
							var endAnimPos = startAnimPos + anim.duration;							
							
							// if scroll is before start of animation, set to start value
							if (scrollLeft < startAnimPos)  setProperty(anim.element, anim.property, anim.startVal);
							
							// if scroll is after end of animation, set to end value
							else if (scrollLeft > endAnimPos) {
								setProperty(anim.element, anim.property, anim.endVal);
								if (blocks[i].pin) {
									blocks[i].block
											.css('position', 'absolute')
							...
							// otherwise, set value based on scroll
								...
								var animPercent = (scrollLeft - startAnimPos) / anim.duration;
		...
		function getCurrBlockIndex(scrollLeft) {
			var currBlockIndex = 0;
			for (var i=0; i<blocks.length; i++) {
				// check if block is in view
				if (blocks[i].left <= scrollLeft - scrollorama.settings.offset)  currBlockIndex = i;
			}
		...
		scrollorama.animate = function(target) {
			/*
				...
				baseline	= left (default, when block reaches left extreme of viewport) or rght (when block first comies into view)
			*/
			...
			for (i=1; i<arguments.length; i++) {
				...
				if (anim.baseline === undefined) {
					if (anim.pin || targetBlock.pin || targetIndex === 0)	anim.baseline = 'left';
					else													anim.baseline = 'right';
				}
				...
				targetBlock.animations.push({
					...
					baseline: anim.baseline !== undefined ? anim.baseline : 'right'
				});
				...
				if (anim.pin) {
					if (targetBlock.pin < anim.duration + anim.delay) {
						...
						for (var j=targetIndex+1; j<blocks.length; j++) {
							blocks[j].left += offset;
							blocks[j].block.css('left', blocks[j].left);
						}
					}
				}
			}
		...
		scrollorama.getScrollpoints = function() {
			var scrollpoints = [];
			for (var i=0; i<blocks.length; i++) {
				scrollpoints.push(blocks[i].left);
				// go through the animations for each block
				if (blocks[i].animations.length && blocks[i].pin > 0) {
					for (var j=0; j<blocks[i].animations.length; j++) {
						var anim = blocks[i].animations[j];
						scrollpoints.push(blocks[i].left + anim.delay + anim.duration);
					}
				}
			}
